...
public:
	...
	inline int toEven(int iValue) { return iValue + (iValue & 1); }
	float toEven(float flValue);
	const char* GetKeyNameVK(int vk);
	bool parseULL(const std::string& s, unsigned long long& value);
	bool IsTaskbarAutoHideEnabled();

	template<typename T>
	inline T EMA(T avg, T newValue, unsigned int fraction)
	{ 
		if (!avg)
			return newValue;
		...
		return (avg * (fraction - 1) + newValue) / fraction;
	}
};